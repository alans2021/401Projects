...
public class PlayerList {
	...
	public PlayerList(String s){
		try{
			text = new File(s); 
			sc = new Scanner(text);
			while(sc.hasNext()){ //If there's another line in file, and that line has info, proceed
				String n = sc.next(); //Info from each line used to make a Player object
				int r = sc.nextInt();
				int w = sc.nextInt();
				int l = sc.nextInt();
				String p = sc.nextLine().trim(); //Remove trailing and leading whitespace
				players.add( new Player(n, r, w, l, p));
			}
	...
	public void updateStats(){ //For each game, stats are updated
		totR = 0; totW = 0; totL = 0; //Everything resets to zero
		for (int i = 0; i < players.size(); i++){
			totR += players.get(i).getRounds();
			totW += players.get(i).getWins();
			totL += players.get(i).getLoss();
	...
	public void saveList(){
		try{
			...
			for(int i = 0; i < players.size(); i++){
				...
				fileOut.println(p.getName() + " " + p.getRounds() + " " + p.getWins() + " " + p.getLoss() + " " + p.getPassword());
			}
			fileOut.close();
		}
		catch(IOException e){
