...
public class Lab7
{
	...
	public static void loadData(MovieDB movies) throws IOException
	{
		...
		for(int i = 0; i < num; i++){
			...
			Movie m = new Movie(title, direct, studio, gross);
			movies.addMovie(m);
		}
	...
	public static void getCommands(MovieDB movies)
	{
		...
		while (true)
		{
			...
			else if (choice.equals("2"))
			{
				...
				String blah = inScan.nextLine();
				temp = new Movie(name, director, studio, gross);
			...
			else if (choice.equals("3"))
			{
				...
				System.out.println("Movie name?");
				String name = inScan.nextLine();
				if(movies.findMovie(name) == null)
					System.out.println(name + " was not found");
				else{
					System.out.println(movies.findMovie(name).toString());
				}
			}
			else
			{
